...
def calculate_similarities(source_sentence, lines):
    ...
    source_embedding = mean_pooling(model_output, encoded_input['attention_mask'])

    similarities = []
    for line in lines:
        # Tokenize and encode each line
        encoded_input = tokenizer([line], padding=True, truncation=True, return_tensors='pt')
        with torch.no_grad():
            model_output = model(**encoded_input)
        line_embedding = mean_pooling(model_output, encoded_input['attention_mask'])

        # Calculate cosine similarity
        similarity = cosine_similarity_score(source_embedding.numpy(), line_embedding.numpy())
...
def similarity():
    sourcesentence = request.args.get('source_sentence')
    with open('konularTumu.txt','r')as f:
        lines = [line.rstrip() for line in f]
    similarities = calculate_similarities(sourcesentence, lines)
